...
const copyAssetsPlugin = {
  ...
  setup(build) {
    build.onEnd(() => {
      ...
      try {
        execSync('node scripts/sync-i18n-json.js', { stdio: 'inherit' });
        if (!existsSync('dist/i18n')) mkdirSync('dist/i18n', { recursive: true });
        cpSync('src/i18n', 'dist/i18n', { recursive: true });
        cpSync('public', 'dist', { recursive: true });
        execSync('node scripts/generate-localized-pages.js', { stdio: 'inherit' });
      } catch (err) {
...
async function serveStaticDevDist(rootDir = 'dist', defaultPort = 4173) {
  ...
  const server = createServer((req, res) => {
    ...
    const directoryIndexPath = resolve(join(fsPath, 'index.html'));
    const cleanHtmlPath = resolve(join(distRoot, normalize(`${requestPath}.html`)));
    const nestedIndexPath = resolve(join(distRoot, normalize(requestPath), 'index.html'));

    if (targetIsDir && existsSync(directoryIndexPath)) {
      targetPath = directoryIndexPath;
    } else if (!targetExists && isSpaRoute) {
      if (existsSync(cleanHtmlPath)) {
        targetPath = cleanHtmlPath;
      } else if (existsSync(nestedIndexPath)) {
        targetPath = nestedIndexPath;
      } else {
