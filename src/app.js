...
import { trackEvent, summarizeFormats } from './analytics.js';
...
function setupLanguageSwitch() {
    ...
    select.addEventListener('change', async () => {
        ...
        const targetPath = i18n.getLocalePath(newLocale);
        if (window.location.pathname !== targetPath) {
            i18n.persistLocale(newLocale);
...
function setupEventListeners() {
    ...
    document.addEventListener('paste', (e) => {
        ...
        if (files.length > 0) handleFiles(files, 'paste');
    });
...
function handleFileSelect(e) {
    handleFiles(Array.from(e.target.files), 'picker');
}

function handleFiles(files, source = 'picker') {
    setStatusMessage('');
    ...
    trackEvent('file_selected', {
        ...
        upload_mode: validFiles.length > 1 ? 'batch' : 'single',
        formats: summarizeFormats(validFiles)
    });
    ...
    if (validFiles.length === 1) {
        ...
        trackEvent('batch_process_start', {
            source,
            file_count: validFiles.length,
            formats: summarizeFormats(validFiles)
        });
...
function getProcessingOutcome(item) {
    return isConfirmedWatermarkDecision(item) ? 'removed' : 'skipped';
}
...
async function processSingle(item) {
    try {
        ...
        if (getProcessingOutcome(item) === 'removed') {
            trackEvent('process_success', {
                processing_mode: 'single',
                file_count: 1,
                outcome: 'removed',
                image_format: summarizeFormats([item.file])
            });
        }
        ...
        console.error(error);
        trackEvent('image_processing_failed', {
            processing_mode: 'single',
            file_count: 1,
...
async function processQueue() {
    ...
    const completedItems = imageQueue.filter((item) => item.status === 'completed');
    if (completedItems.length > 0) {
        const removedCount = completedItems.filter((item) => getProcessingOutcome(item) === 'removed').length;
        const skippedCount = completedItems.length - removedCount;
        if (removedCount > 0) {
            trackEvent('process_success', {
                processing_mode: 'batch',
                file_count: imageQueue.length,
                completed_count: completedItems.length,
                removed_count: removedCount,
                skipped_count: skippedCount,
                error_count: imageQueue.filter((item) => item.status === 'error').length,
                formats: summarizeFormats(imageQueue.map((item) => item.file))
            });
        }
    }
}
...
function downloadImage(item) {
    trackEvent('download_click', {
        ...
        outcome: getProcessingOutcome(item),
        image_format: summarizeFormats([item.file])
    });
...
async function downloadAll() {
    ...
    trackEvent('download_click', {
        ...
        removed_count: completed.filter((item) => getProcessingOutcome(item) === 'removed').length,
        skipped_count: completed.filter((item) => getProcessingOutcome(item) === 'skipped').length
    });
