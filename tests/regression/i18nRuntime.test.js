...
function setupRuntime({
  ...
  setGlobal('window', { location: { pathname } });
...
test('should infer locale from localized path before stored or navigator locale', async () => {
  setupRuntime({
    savedLocale: 'en-US',
    ...
    fetchImpl: async () => ({
      ok: true,
      async json() {
        return { title: 'Titre', 'nav.upload': 'Importer' };
      },
    }),
  });
...
test('init should fallback to en-US and always clear loading class when locale load fails', async () => {
  ...
  assert.deepEqual(requested, ['/i18n/fr-FR.json', '/i18n/en-US.json']);
...
test('should provide locale rotation for all supported locales', async () => {
  ...
  assert.equal(i18n.getNextLocale('zh-TW'), 'en-US');
  assert.equal(i18n.getNextLocale('en-US'), 'es-ES');
  assert.equal(i18n.getNextLocale('pt-BR'), 'it-IT');
  assert.equal(i18n.getLocaleShort('pt-BR'), 'PT');
  assert.equal(i18n.getLocalePath('ar-SA'), '/ar/');
});
